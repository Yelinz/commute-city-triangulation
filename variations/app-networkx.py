...
with station_distance_container:
    ...
    if selection["last_object_clicked_tooltip"]:
        ...
        if id_match:
            ...
            network.add_nodes_from(selected_route["stop_sequence"], subset=1)
            edges = list(
                zip(
            ...
            plot = figure(
                ...
                title="Stations on selected route",
            )
            ...
            graph_renderer.edge_renderer.glyph = MultiLine(line_alpha=1, line_width=2)

            plot.renderers.append(graph_renderer)

            # Node lables
            node_source = graph_renderer.node_renderer.data_source
            node_source.data["names"] = selected_route["stop_name"]
            ...
            edge_source.data["names"] = [
                "%d-%d" % (x, y)
                for (x, y) in zip(edge_source.data["start"], edge_source.data["end"])
            ]
            ...
            edge_code = """
                ...
                return result
            """
            edge_xcoord = CustomJSTransform(
                v_func=edge_code % "0",
                args=dict(provider=graph_renderer.layout_provider, source=edge_source),
            )
            edge_ycoord = CustomJSTransform(
                v_func=edge_code % "1",
                args=dict(provider=graph_renderer.layout_provider, source=edge_source),
            )
            edge_labels = LabelSet(
                x=transform("start", edge_xcoord),
                y=transform("start", edge_ycoord),
                text="names",
                text_font_size="12px",
                x_offset=-5,
                y_offset=-15,
